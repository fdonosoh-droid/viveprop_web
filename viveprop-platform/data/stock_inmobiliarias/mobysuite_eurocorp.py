...
TOKENS: si el script da error 401/403, copiá un nuevo cURL desde DevTools
y actualizá BEARER_TOKEN y CAPTCHA_TOKEN abajo.
"""
...
STATE_COLORS = {
    "DISPONIBLE":  "C6EFCE",  # verde claro
    "VENDIDO":     "FFCCCC",  # rojo claro
    "RESERVADO":   "FFEB9C",  # amarillo
    "BLOQUEADO":   "D9D9D9",  # gris
    "PROMESADO":   "FCE4D6",  # naranja claro
}
...
COLUMNS = [
    ...
    ("Tipo",               "tipoBien",               14),
    ("Piso",               "piso",                    6),
    ("Orientación",        "orientacion",            12),
    ("Estado",             "estadoBien",             14),
    ("Tipología",          "tipologiaDepartamentos", 22),
    ("Dormitorios",        "numeroDeDormitorios",    12),
    ("Baños",              "numeroDeBanos",           8),
    ("Sup. Interior m²",   "superficieInterior",     16),
    ("Sup. Terraza m²",    "superficieTerraza",      15),
    ("Sup. Total m²",      "superficieTotal",        14),
...
def get_all_projects():
    ...
    while True:
        ...
        if resp.status_code == 401:
            print("ERROR 401: Token vencido. Actualizá BEARER_TOKEN y CAPTCHA_TOKEN.")
            sys.exit(1)
        ...
        print(f"  Página {page + 1}/{total_pages} — {len(projects)} proyectos")
...
def write_asset_row(ws, row_num, asset, project):
    ...
    row_color = STATE_COLORS.get(estado, "FFFFFF")
...
def build_excel(projects, all_assets_by_project, output_path):
    ...
    for project in projects:
        ...
        if not assets:
            continue
        sheet_name = project["nombreProyecto"][:31]
        ws_p = wb.create_sheet(title=sheet_name)
        for col, (header, _, width) in enumerate(COLUMNS[1:], 1):  # sin columna Proyecto
        ...
        for r, asset in enumerate(assets, 2):
            for col, (_, field, _) in enumerate(COLUMNS[1:], 1):
                estado = asset.get("estadoBien", "")
                fill = PatternFill(start_color=STATE_COLORS.get(estado, "FFFFFF"),
                                   end_color=STATE_COLORS.get(estado, "FFFFFF"),
                                   fill_type="solid")
                cell = ws_p.cell(row=r, column=col, value=asset.get(field) if field != "monedaPrincipal"
                                 else project.get("monedaPrincipal", "UF"))
                cell.fill = fill
...
def main():
    print("=" * 55)
    print("  MobySuite — Descarga de Stock EUROCORP")
    print("=" * 55)

    print("\n[1/3] Obteniendo lista de proyectos...")
    projects = get_all_projects()
    print(f"      Total: {len(projects)} proyectos\n")

    print("[2/3] Descargando unidades por proyecto...")
    all_assets = {}
    total_units = 0
    for p in projects:
        ...
        print(f"      {p['nombreProyecto']:<40} {len(assets):>4} unidades  ({disponibles} disponibles)")

    print(f"\n      Total unidades descargadas: {total_units}")

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = f"{OUTPUT_DIR}/eurocorp_stock_{ts}.xlsx"

    print(f"\n[3/3] Generando Excel: {output_path}")
    build_excel(projects, all_assets, output_path)
    print(f"\nListo! Archivo guardado en: {output_path}")
    print(f"  Hojas: Resumen + Consolidado + {len(projects)} proyectos individuales")
